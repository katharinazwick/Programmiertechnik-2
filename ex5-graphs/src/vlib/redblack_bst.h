...
// Implements the class for a symbol table based on (balanced) red-black binary
// search trees
...
class RedBlackBST : public ST<Key, Value> {
    ...
    struct Node {
        ...
        Node(const Key& k, const Value& v, const bool _red, const int s)
            : key(k),
              val(v),
              left(nullptr),
              right(nullptr),
    ...
    Node* deep_copy_tree(const Node* n) {
        if (n) {
            Node* node = new Node(n->key, n->val, n->red, n->size);
    ...
    bool is_red(const Node* n) const { return (n ? n->red : false); }
    ...
    Node* put(Node* n, const Key& key, const Value& val) const {
        if (n == nullptr) {
            Node* node = new Node(key, val, true, 1);
            node->left = nullptr;
            node->right = nullptr;
            return (node);
        }
    ...
    // flip the colors of the link pointing to the node and its two children
    // links
    ...
    // Assuming that n has a red link pointing into it and both n->left and
    // n->left->left are black, make n->left or one of its child links red.
    ...
    // Assuming that n has a red link pointing into it and both n->right and
    // n->right->left are black, make n->right or one of its child links red.
   ...
   public:
    ...
    // destructor
    virtual ~RedBlackBST() { free_tree(root); }
    ...
    void remove(const Key& key) {
        ...
        if (!is_red(root->left) && !is_red(root->right)) root->red = true;
